	...
	public void handleServer(Session session, AnimationMessage message) {
		...
		switch (message.getAnimation()) {
			case AnimationMessage.ANIMATION_CROUCH:
				session.send(new EntityActionMessage(player.getId(), EntityActionMessage.ACTION_CROUCH));
				//TODO Set this in VanillaActionController as apparently any entity can crouch?
				break;
			case AnimationMessage.ANIMATION_UNCROUCH:
				session.send(new EntityActionMessage(player.getId(), EntityActionMessage.ACTION_UNCROUCH));
				//TODO Set this in VanillaActionController as apparently any entity can crouch?
				break;
			default:
		}
