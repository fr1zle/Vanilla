...
public final class PlayerDiggingMessageHandler extends MessageHandler<PlayerDiggingMessage> {
	...
	public void handleServer(Session session, Player player, PlayerDiggingMessage message) {
		...
		Block block = w.getBlock(x, y, z, player);
		...
		InventoryBase inv = ((VanillaPlayer) player.getController()).getInventory();
		...
		if (state == PlayerDiggingMessage.STATE_START_DIGGING) {
			...
			if (!isInteractable && heldItem == null) {
				...
				heldItem.getSubMaterial().onInteract(player, Action.LEFT_CLICK);
			} else {
				// interacting with block using item
				heldItem.getSubMaterial().onInteract(player, block, Action.LEFT_CLICK, clickedFace);
				blockMaterial.onInteractBy(player, block, Action.LEFT_CLICK, clickedFace);
			}
			...
			if (isInteractable) {
				...
				if (fire) {
					...
					VanillaNetworkUtil.playBlockEffect(block, player, PlayEffectMessage.Messages.RANDOM_FIZZ);
					...
					sendPacketsToNearbyPlayers(player, player.getViewDistance(), pem);
				}
			}
